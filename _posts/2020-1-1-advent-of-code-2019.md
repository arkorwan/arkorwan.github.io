...
ผมค่อยๆ สร้าง [class IntCode](code TBD)[^0] เพิ่มความสามารถ ปรับแต่งไปเรื่อยๆ ตามที่ spec ที่ได้เพิ่มมา ท้ายที่สุดได้คลาสที่มี public method ดังนี้
...
อีกอย่างก็คือ ข้อนี้เป็นตัวอย่างชั้นดีของโจทย์ที่ไม่ค่อยเจอในที่อื่นๆ
คือถ้าเป็นโจทย์ที่ตรวจด้วย automate checker ตามปกติ เค้าก็จะต้องบอกมาอย่างละเอียดว่า ตัวอักษรแต่ละตัวนี่มันหน้าตาเป็นยังไง
เทียบกับโจทย์แบบนี้ที่มันมีความลึกลับอยู่ว่าเราจะได้ result หน้าตายังไงหว่า ตรงนี้ก็เป็นเสน่ห์อย่างหนึ่งของ Advent of Code เลย
...
ซึ่งงานแบบนี้มันเหมาะให้เครื่องทำมากกว่ามานั่งจิ้มเองแน่ๆ สุดท้ายก็เลยต้องเขียนเป็น [code](TBD) แก้อัตโนมัติอยู่ดี
...
แต่ต้องบังคับให้มันเดินไป ก็เลยทำ flood-fill ลำบากละ
ก็เลยเปลี่ยนไปคิดว่างานนี้ต้องเขียน maze solver แทนจริงๆ มั้ยนะ แบบให้หุ่น track กำแพงด้านซ้ายไว้ตลอดอะไรทำนองนี้

แต่ว่า ไหนๆ เราก็มีโค้ดเขียน interface จากข้อก่อนหน้านี้แล้ว ก็เอามาทำเป็น [interactive code](TBD) อีกสักข้อเพื่อให้เราคุมหุ่นแบบ manual ไปเลยดีกว่า
คือถ้าเดินได้ก็จะวาดเป็นจุด (.) ออกมาว่าเดินไปแล้วนะ ถ้าเดินชนกำแพงก็วาดกำแพง (■) ออกมาเลย

งานนี้ ก็คล้ายๆ กับข้อที่แล้ว คือแก้ด้วยมือมันก็แก้ได้ แต่ให้คอมทำน่าจะดีกว่า ... แต่คราวนี้ผมลุยถึก manual ไปนี่แหละ จนวาด map ออกมาได้แบบนี้ เสร็จแล้วต้องรีบเซฟเก็บไว้ ฮ่าๆ
...
ส่วนคำตอบ เราโชคดีที่โจทย์ปรานี ให้ maze มาโดยที่ไม่มีการวนลูป ดังนั้นเราก็แค่นับระยะ +1 ไปจาก cell ก่อนหน้าไปเรื่อยๆ ไม่ต้องย้อนกลับมา update ค่าเก่า ก็เลยง่ายหน่อย
อันนี้ก็เป็น hidden requirement ชนิดหนึ่งของ Advent of Code
...
เลยไม่ได้นึกว่า ที่จริงเราสร้าง copy ของโปรแกรมใน state สำหรับแต่ละ cell ขึ้นมาก็ได้
...
- `####` → NOT JUMP! กรณีนี้เรามองได้ว่า การกระโดดไปยังช่อง i+4 เราอาจจะเลือกเดินไป 3 ครั้ง ก็ได้ผลเหมือนกัน
แต่ถ้าเลือกเดินไป เราจะมีข้อมูลเพิ่มขึ้นเรื่อยๆ ในการเดิน 2 ครั้งแรก และสามารถเปลี่ยนใจกระโดดหนีได้ก่อนจะไปถึงช่อง i+4
...
[![p25.gif](https://i.postimg.cc/L5yfnZ2W/p25.gif#center) *เก็บ Infinite loop ก็พังสิ! (แต่เรา restore ได้)*](https://postimg.cc/N2HLnFy8)
